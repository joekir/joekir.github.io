...
I spoke to some authentication experts about my previous ideas on this topic. (*These don't need to be read to understand this post...*)

* [Serverside One-Time-Pad (Part1)](/2016/08/02/serverside-otp-part1/)
* [Serverside One-Time-Pad (Part2)](/2016/08/05/serverside-otp-part2/)

This compelled me to the think of some improvements.

The 2nd iteration was an improvement as it provided protection against façade-forwarding of the otp token. But still it required the client to protect the confidentiality of a secret. Ideally I would like it if the client only had to guarantee integrity, but didn't have to guarantee confidentiality [[1](https://en.wikipedia.org/wiki/Information_security#Key_concepts)]. 

In brainstorming this further, I recalled a scheme devised by the brilliant, Turing-award winning [Leslie Lamport](https://en.wikipedia.org/wiki/Leslie_Lamport) (was that introduction fanboi-ish enough?!). He devised something that was later used for [S/Key](https://en.wikipedia.org/wiki/S/KEY) known as [Hash Chains](https://en.wikipedia.org/wiki/Hash_chain). To summarize, the server maintains a forward hash of a secret, and the client presents what the pre-image of that is (a pre-image is a hard thing to attack - see [previous blog post](/2016/05/05/humbled-by-md5/)).
...
$$IP|hash_{998}$$ is difficult (Even with poor choices like MD5, again see [previous post](/2016/05/05/humbled-by-md5/)). Therefore if the client can guarantee anti-tamper of the hash that it is storing, but not confidentiality, it will still be able to securely authenticate the server.
...
3. It may look at first like binding the IP address into the hashing scheme is overly rigid. It is. But this could be loosened to an dedicated IP range. Which in IPv6 which would me much less likely to change over time.

#### Caveats

* The initial client sign-up is not at all protected via this scheme. [HSTS](https://en.wikipedia.org/wiki/HTTP_Strict_Transport_Security) is a great mitigation for this, but only if the site is on the browser [preload list](https://hstspreload.appspot.com/) and only if the user doesn't bypass the warnings.
